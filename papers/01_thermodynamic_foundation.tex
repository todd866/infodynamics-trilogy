...
To verify the geometric maintenance bound (Eq.~\ref{eq:dlb}), we simulate random projections from $D_{\mathrm{high}} = 50$ to various target dimensions $D_{\mathrm{low}}$ (Figure~\ref{fig:landauer_verification}). For each projection, we compute the informational cost $\ln 2 \cdot \Delta I$ and the geometric contraction cost $C_{\Phi}$ from the projection Jacobian. The simulation confirms: (1) both costs scale linearly with the number of dimensions removed, (2) the total bound $W_{\mathrm{bound}}$ is always less than or equal to the actual dissipation $W_{\mathrm{actual}}$, and (3) the bound becomes tight (efficiency $\to 1$) for optimal processes. The $R^2 = 0.97$ correlation between dimensions removed and work required validates the scaling predicted by Eq.~(\ref{eq:dlb}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/fig_landauer_verification.pdf}
    \caption{Numerical verification of the geometric maintenance bound. \textbf{(A)} The informational term (bits removed) and geometric term (Jacobian contraction cost) both scale linearly with dimensional reduction. \textbf{(B)} Actual dissipation always exceeds the theoretical bound---the bound is tight. \textbf{(C)} Process efficiency approaches optimal (reversible) for well-chosen projections. Simulations use 10,000 samples and 20 trials per target dimension.}
    \label{fig:landauer_verification}
\end{figure}
