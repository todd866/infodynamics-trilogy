...
\begin{enumerate}
    ...
    \item \textbf{Abiogenesis and code complexity}: High-dimensional systems find solutions to constraint-satisfaction problems at costs far below naive combinatorial estimates \cite{todd2025intelligence}. This applies to the early universe: if prebiotic chemistry operated in a high-dimensional state space (many coupled molecular species, reaction networks, spatial degrees of freedom), then complex codes---genetic, metabolic, regulatory---could emerge not despite thermodynamic improbability but \emph{because} high-dimensional exploration makes them accessible. The genetic code is ``found'' the same way solutions are found in high-dimensional neural spaces: through thermodynamically cheap exploration of constraint surfaces, not through improbable random assembly. This predicts that the complexity of early codes should exceed what naive attractor-basin combinatorics would suggest---a testable claim against origin-of-life models that assume low-dimensional assembly dynamics.
\end{enumerate}
...
I. Todd, ``Dimensional constraints on biological intelligence: Why high-dimensional systems solve problems that exhaust low-dimensional ones,'' \textit{BioSystems} (2025, in press).
