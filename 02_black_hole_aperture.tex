...
\textbf{Contributions.} We (1) define dimensional aperture and derive two effective dimensions ($k_w$, $k_{\mathrm{dyn}}$); (2) derive a Fisher-speed time proxy $\dot{\tau}$ from a Gaussian observation model; (3) simulate coupled oscillators showing horizon-like freezing and thermodynamic accumulation; and (4) argue that the results generalize across aperture families. An outlook section sketches a speculative analogy to gravitational wave relaxation.
...
An observer's aperture is a weight vector $\mathbf{w} \in [0,1]^N$ determining access to each positional mode. We define the observation operator $A = \mathrm{diag}(\sqrt{w_1}, \ldots, \sqrt{w_N})$, so:
\begin{equation}
    \mathbf{x}_{\mathrm{obs}} = A\mathbf{x} = \sqrt{\mathbf{w}} \odot \mathbf{x}
\end{equation}
The clock rate (Eq.~5) uses velocities $\dot{\mathbf{x}}$ with the same operator.
\end{definition}
...
\textbf{Dynamical dimension} (from observed state statistics):\footnote{This is the same quantity as $D_{\mathrm{eff}}$ (the participation ratio) in the companion paper \cite{todd2025infodynamics}; we use $k_{\mathrm{dyn}}$ here to emphasize its role as a dynamical observable computed from time-series covariance.}
\begin{equation}
...
The observation model is: the observer receives $\mathbf{y} = A\mathbf{x} + \boldsymbol{\eta}$ where $\boldsymbol{\eta} \sim \mathcal{N}(0, \sigma_0^2 I)$ is isotropic measurement noise. The KL divergence between observations at nearby states $\mathbf{x}$ and $\mathbf{x} + \mathrm{d}\mathbf{x}$ is:
\begin{equation}
    D_{\mathrm{KL}} = \frac{1}{2\sigma_0^2} \|A\,\mathrm{d}\mathbf{x}\|^2 = \frac{1}{2\sigma_0^2} \sum_i w_i \,\mathrm{d}x_i^2
\end{equation}
This identifies the Fisher information metric as $G = A^\top \Sigma^{-1} A = \mathrm{diag}(w_1, \ldots, w_N)/\sigma_0^2$. Setting $\sigma_0 = 1$ (absorbing noise scale into $\mathbf{w}$), the observed coordinates $\mathbf{x}_{\mathrm{obs}} = \sqrt{\mathbf{w}} \odot \mathbf{x}$ are precisely the coordinates that make the Fisher metric diagonal with entries $w_i$.

\textbf{Why $\sqrt{w}$ in Eq.~(1)?} This ensures consistency between signal attenuation and Fisher geometry. If we attenuated by $w$ directly (not $\sqrt{w}$), the Fisher metric would scale as $w^2$, not $w$. The $\sqrt{w}$ form guarantees that a mode with weight $w_i$ contributes $w_i$ to both the observed variance and the metric---signal access and distinguishability degrade together.

The Cram\'{e}r-Rao bound formalizes this: for any unbiased estimator $\hat{x}_i$ of mode $i$, the variance satisfies $\mathrm{Var}(\hat{x}_i) \geq \sigma_0^2/w_i$. As $w_i \to 0$, variance diverges---the mode becomes statistically unmeasurable regardless of estimator choice.
...
\textbf{Definition (operational time dilation):} In this framework, ``time dilation'' means reduced \emph{operational time resolution} for an observer whose physics is mediated by constrained channels. It is not a claim about coordinate time slowing; it is a claim about the rate at which distinguishable records can form.}}
\medskip
...
This connects to \cite{todd2025infodynamics}: maintaining a low-dimensional description of a high-dimensional system requires work. Horizons are where this work grows large. The aperture framework also extends prior work on timing inaccessibility \cite{todd2025timing} and coherence time \cite{todd2025coherence}: the same constraints that limit temporal resolution in biological substrates---where coherence time scales exponentially with coordination depth---apply, in generalized form, to any information channel approaching a horizon. When the aperture closes, fewer dimensions remain accessible, and the time required to achieve coherent state discrimination grows.
...
\end{equation}
plus weak damping $\dot{p}_i \to \dot{p}_i - \gamma p_i$. The damping ensures bounded motion over long integration times; we do not add stochastic forcing (Langevin noise), so this is an energy sink rather than a thermal bath. The thermodynamic costs computed below ($S_{\mathrm{acc}}$, $Q$) track the observer's accessible description volume, not the bath entropy.

\textbf{Parameters:} $N=50$ oscillators, coupling $\kappa=0.1$, damping $\gamma=0.01$, timestep $\Delta t = 0.01$ (LSODA integration via \texttt{scipy.integrate.odeint}), $N_{\mathrm{steps}}=1000$. Initial conditions: positions and momenta drawn i.i.d.\ from $\mathcal{N}(0,1)$. Covariance estimated over a sliding window of 200 samples after 100-sample warmup. Plots show single representative runs; qualitative features are robust across seeds.

Two observers watch the same dynamics:

\textbf{External observer}: Aperture depends on ``radius'' $r \in (r_s, 1]$:
\begin{equation}
    w_k(r) = \exp\left(-\frac{f_k \cdot r_s}{r - r_s}\right), \quad f_k = \frac{\omega_k - \omega_{\min}}{\omega_{\max} - \omega_{\min}}
\end{equation}
where $r_s$ is the horizon radius and $\omega_k$ are the normal mode frequencies of the coupled oscillator system. Higher-frequency modes are suppressed faster as $r \to r_s$ (approaching horizon).

The physical interpretation: external observers have \emph{finite time resolution} (sampling bandwidth). Near-horizon gravitational redshift pushes interior dynamics below that resolution threshold, making high-frequency components inaccessible first. We diagonalize the coupled oscillator system to obtain actual normal mode frequencies $\omega_k$, ordering modes from low to high frequency. This is more physical than an index-based proxy: the suppression is tied to actual oscillation frequencies, not arbitrary mode labels.

\textbf{Why this functional form.} Gravitational redshift scales as $\sqrt{1 - r_s/r}$; emitted frequency $\nu$ is received as $\nu' = \nu \sqrt{1 - r_s/r}$. High-frequency modes fall below the observer's bandwidth threshold before low-frequency modes do. The exponential form in Eq.~(10) captures this near-horizon divergence: as $r \to r_s$, the exponent diverges and all mode weights vanish. This is analogous to renormalization group flow: as you coarse-grain toward the infrared, ultraviolet modes are integrated out first. We verify below that alternative monotone families (power-law, sigmoid) produce similar phenomenology, confirming robustness.
...
\subsection{Outlook: Channel Relaxation}

\textbf{Speculative note:} Any bounded information channel exhibits damped relaxation when abruptly perturbed. If spacetime geometry reflects information-accessibility constraints, then perturbations to those constraints would produce relaxation with characteristic timescales. We note this as a structural observation---we are not modeling gravitational wave strain or solving Einstein's equations. The connection to actual gravitational wave physics remains an open question for future work.

\subsection{Quantitative Schwarzschild Comparison}

How closely does the aperture model match actual GR predictions? Figure~\ref{fig:schwarzschild} compares the simulated $\dot{\tau}(r)$ against the Schwarzschild time dilation factor $\sqrt{1 - r_s/r}$. With a redshift-motivated aperture family (Eq.~10), $\dot{\tau}(r)$ can be fit to Schwarzschild with high correspondence ($R^2 \approx 0.99$ in typical runs).

This is suggestive: we derived $\dot{\tau}$ from Fisher geometry without solving Einstein's equations. However, the close fit is partly by construction---the aperture functional form (Eq.~10) was chosen because it has the right divergence structure at $r \to r_s$. The claim is not that we \emph{derived} Schwarzschild time dilation, but that an information-geometric framework with horizon-like boundary conditions \emph{reproduces} it. This supports the interpretation that spacetime geometry may be downstream of information-accessibility constraints, while acknowledging the phenomenological nature of the aperture choice.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/fig_schwarzschild_comparison.pdf}
    \caption{Schwarzschild comparison. \textbf{Left}: Time rate $\dot{\tau}$ vs radius for the aperture model and Schwarzschild prediction. \textbf{Right}: Direct comparison showing high correlation (see panel title for exact $R^2$). The aperture model reproduces the Schwarzschild curve; however, the fit is partly by construction (see text). The claim is phenomenological correspondence, not derivation.}
...
Is the phenomenology an artifact of our specific aperture function (Eq.~10)? The qualitative results should be robust to functional form. Alternative monotone families such as:
\begin{itemize}
    \item Power-law: $w_i(r) = r^{\beta f_i}$
    \item Sigmoid: $w_i(r) = \sigma(\alpha(r - r_s))^{f_i}$
\end{itemize}
would produce the same generic behavior: $k_{\mathrm{dyn}}$ collapses as channels close, $\dot{\tau}$ drops as variance compresses, $Q$ accumulates, and infalling observers see none of these effects. The specific functional form affects quantitative details (how fast, at what radius) but not the phenomenon itself. Any monotone frequency-dependent filter that closes at a boundary produces horizon-like phenomenology---this follows from the structure of the definitions, not the particular exponential form.
...
\noindent As noted in Section 4.3, channel relaxation dynamics remain speculative---we do not claim to model gravitational wave strain. Whether spacetime geometry actually tracks channel capacity in this way is an open question.
...
\begin{enumerate}
    ...
    \item \textbf{Schwarzschild mismatch.} Our simulation achieves high correspondence ($R^2 \approx 0.99$) with Schwarzschild time dilation. If more precise simulations or different aperture families systematically deviated from GR predictions, the aperture interpretation would require modification or abandonment.
...
I. Todd, ``Timing inaccessibility and the projection bound: Resolving Maxwell's demon for continuous biological substrates,'' \textit{BioSystems} \textbf{258}, 105632 (2025).

\bibitem{todd2025coherence}
I. Todd, ``Coherence time in neural oscillator assemblies sets the speed of thought,'' \textit{BioSystems} (2025, under review). Preprint: \url{https://github.com/todd866/coherence-time-biosystems}
